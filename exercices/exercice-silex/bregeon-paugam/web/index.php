    ...
    if ($form->isValid()) {
      ...
      if (mail($contactEmail, 'Contact Form', $formdata['message'], $headers))
      {
        echo '  <div class="alert alert-success">
                  ...
                  <p>Merci de nous avoir écrit ! Votre email a été envoyé avec succès et nous vous répondrons dans les meilleurs délais.</p>
                </div>';      }
...
$app->match('/addsnippets',function(Request $request)
{
  ...
  if($request->getMethod() == 'POST'){
    ...
    if ($state == "success")
      echo '  <div class="alert alert-success">
             <p><strong>Votre snippet a été envoyé !</strong></p>
            <p>Merci de votre contribution. Votre snippet a été envoyé avec succès.</p>
              </div>';
    elseif ($state == "NON")
      echo 'Probleme';
  }
