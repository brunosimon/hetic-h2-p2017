<?php
	...
	if(!empty($_POST['country']))
		$exec = $pdo->exec('UPDATE users SET country = \''.$_POST["country"].'\' WHERE login = \''.$_COOKIE['login'].'\'');

	//$query = $pdo->query('SELECT * FROM users WHERE login = '.$_COOKIE['login'].'');
	...
	*/
	...
	foreach($stadiums as $_key => $_stadium):
		if(!empty($_POST[$_key])){
			$exec = $pdo->exec('UPDATE users SET '.$_key.' = \''.$_POST[$_key].'\' WHERE login = \''.$_COOKIE['login'].'\'');
			
			//$exec2 = $pdo->exec('UPDATE stadiums SET note = (AVG('.$_key.') FROM users) WHERE name = \''.$_key.'\'');
	...
	foreach($stadiums as $_key => $_stadium):
			$exec = $pdo->exec('SELECT AVG('.$_key.') FROM users');

			echo '<h4>Note moyenne : '.$exec.'</h4>';
	endforeach;


/*

		$query = $pdo->query('SELECT * FROM stadiums');
		$teams = $query->fetchAll();
		$results = array();
...
<body>
	...
	<h4>Welcome <a id="log"><?php echo ($_COOKIE['login']); ?></a>, please complete the following poll :</h4>
	<br /><br />
	<div class="teams">
		...
		<form method="post">
				...
				?>
			<br /><input type="submit" class="btn btn-primary btn-block btn-large" value="Submit"/>
		</form>
	</div>
	...
	<div class="stadiums">
		<h2>Give a note to all stadiums</h2>
		<form method="post">
